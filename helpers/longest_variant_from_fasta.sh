...
##################################
# MODIFIED FOR MAIZE NOMENCLATURE
##################################
...
split_gene_splicevar() {
  perl -ne 'if(/^(\S+)_T(\d+)\t(\S+)/){print "$1\tzPLACEHOLDERz\t$2\t$3\n"};
            if(/^([^_T]+)\t(\S+)/){print "$1\tzPLACEHOLDERz\tzQz\t$2\n"};
            if(/^(\S+)_T([^\d+])(\d+)\t(\S+)/){print "$1\t$2\t$3\t$4\n"}' |
